...
public:
    ...
    throttle_filt_(std::nullopt) // NEW: 스로틀 필터 상태 초기화
  {
    ...
    sub_safety_status_ = this->create_subscription<std_msgs::msg::String>(
      "/safety_status", 10, std::bind(&PurePursuitNode::onSafetyStatus, this, std::placeholders::_1));
    ...
    if (viz_enable_) {
      ...
      pub_viz_safety_text_ = this->create_publisher<visualization_msgs::msg::Marker>("/pp/safety_text", 1);        // 안전 상태 텍스트
...
private:
  ...
  void onSafetyStatus(const std_msgs::msg::String::SharedPtr msg)
  {
    current_safety_status_ = msg->data;
  }
  ...
  void onTimer()
  {
    ...
    if (viz_enable_) {
      ...
      publishSafetyTextCsv(rob_p); // 안전 상태 텍스트 시각화 추가
  ...
  void publishLdTextCsv(double Ld, const geometry_msgs::msg::Point& robot_center_csv)
  {
    ...
    if (current_mission_state_ == "REVERSE_T" || current_mission_state_ == "REVERSE_PARALLEL") {
      ...
      txt.color.r = 0.0; txt.color.g = 0.0; txt.color.b = 1.0; // 파란색
    }
  ...
  void publishThrottleTextCsv(const geometry_msgs::msg::Point& robot_center_csv)
  {
    ...
    txt.scale.z = 1.8; // 텍스트 크기
    txt.color.a = 1.0;
    if (current_throttle_cmd_ < -0.001) {
      txt.color.r = 1.0; txt.color.g = 0.1; txt.color.b = 0.1; // 빨간색 (음수)
    } else if (std::abs(current_throttle_cmd_) <= 0.001) {
      txt.color.r = 1.0; txt.color.g = 1.0; txt.color.b = 1.0; // 흰색 (0.0)
    } else {
      txt.color.r = 0.2; txt.color.g = 0.8; txt.color.b = 1.0; // 하늘색 (양수)
    }
    txt.pose.position = robot_center_csv;
  ...
  void publishSafetyTextCsv(const geometry_msgs::msg::Point& robot_center_csv)
  {
    visualization_msgs::msg::Marker txt;
    txt.header.frame_id = path_frame_id_;
    txt.header.stamp = this->now();
    txt.ns = viz_ns_;
    txt.id = 10; // ID for safety status text
    txt.type = visualization_msgs::msg::Marker::TEXT_VIEW_FACING;
    txt.action = visualization_msgs::msg::Marker::ADD;
    txt.scale.z = 1.8; // 텍스트 크기
    txt.color.a = 1.0;

    if (current_safety_status_ == "SAFE_OK") {
      txt.color.g = 1.0; // 초록색
    } else {
      txt.color.r = 1.0; txt.color.g = 0.0; txt.color.b = 0.0; // 빨간색 (경고)
    }
    txt.pose.position = robot_center_csv;
    txt.pose.position.y += -9.8; // 미션 텍스트 아래에 표시
    txt.pose.position.z += 0.5;

    char buf[64];
    std::snprintf(buf, sizeof(buf), "/safety_status %s", current_safety_status_.c_str());
    txt.text = std::string(buf);
    pub_viz_safety_text_->publish(txt);
  }
  ...
  rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr    pub_viz_safety_text_; // 안전 상태 시각화용
  ...
  std::string current_safety_status_ = "SAFE_OK";
  float current_throttle_cmd_ = 0.0;
  rclcpp::Subscription<std_msgs::msg::String>::SharedPtr sub_mission_state_;
  rclcpp::Subscription<std_msgs::msg::String>::SharedPtr sub_safety_status_;
