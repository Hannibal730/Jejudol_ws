...
class SerialBridge(Node):
    def __init__(self):
        ...
        # 고정 설정값
        self.throttle_cmd_topic = '/auto_throttle'
        self.steer_cmd_topic = '/auto_steer_angle'
        self.port = DEFAULT_PORT
        self.baud = DEFAULT_BAUD
        self.startup_silence_sec = 3.0  # 시작 후 송신 차단 시간(초)
        ...
        # 재연결 스레드 (open)
        self._recon_th = threading.Thread(target=self._reconnect_loop, args=(self.port, self.baud), daemon=True)
        ...
        # 구독자
        self.create_subscription(Float32, self.throttle_cmd_topic, self.cb_throttle, 10)
        self.create_subscription(Float32, self.steer_cmd_topic, self.cb_steer, 10)

        self.get_logger().info(
            f"Subscribed to {self.throttle_cmd_topic} and {self.steer_cmd_topic} → Serial({self.port}@{self.baud}), "
