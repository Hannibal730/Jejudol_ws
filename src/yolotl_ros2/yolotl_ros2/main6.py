...
def offset_polyline_points(xs, ys, offset_px, direction=1.0, sample_step=20):
    ...
    same_idx_mask = (idx_prev == idx_next)
    dx[same_idx_mask] = 0.0
    dy[same_idx_mask] = 1.0

    norm = np.sqrt(dx * dx + dy * dy) + 1e-6
    nx = dy / norm
    ny = -dx / norm

    sparse_xo = xs[indices] + direction * offset_px * nx
    sparse_yo = ys[indices] + direction * offset_px * ny
