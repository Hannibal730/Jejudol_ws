...
class ConeDetectionNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('bev_params', 'bev_params_0325.npz')
        ...
        self.m_per_pixel_y = 0.0027
        self.y_offset_m = 1.23
        self.m_per_pixel_x = 0.0030
        ...
        self.sub_drivable = self.create_subscription(CompressedImage, '/drivable_area/compressed', self.drivable_callback, 10)

        # 결과 이미지 퍼블리셔
        self.pub_result = self.create_publisher(CompressedImage, '/cone_detection/compressed', 10)

        # [추가] Throttle 제어 퍼블리셔 및 Emergency 상태 변수
        self.pub_emergency = self.create_publisher(Bool, '/emergency', 1)
    ...
    def image_callback(self, msg):
        try:
            ...
            if 2 <= obstacle_count < 4:
                self.emergency_end_time = now + Duration(seconds=0.1)
