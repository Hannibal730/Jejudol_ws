...
#    2-a) 문코스 유지: 기존에 moon_course 진입 상태였다면 (moon_course_hold=True)
...
class DecisionNode(Node):
    ...
    def _get_mission_state(self) -> str:
        ...
        if self.moon_course_hold and (num_lidar_cone != 0):
            return MISSION_MOON_COURSE

        # *문코스 트리거
        if lane_detect and (num_lidar_cone >= self.num_moon_course_threshold):
            self.moon_course_hold = True
            return MISSION_MOON_COURSE

        # 문코스 홀딩 X (if num_lidar_cone == 0:) and emergency O -> emergency
        if self.moon_course_hold == False and self.emergency:
            return MISSION_EMERGENCY
        ...
        if num_lidar_cone >= self.num_static_obstacle_threshold:
