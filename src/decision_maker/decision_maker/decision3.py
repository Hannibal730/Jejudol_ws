...
#    - 아니면 mission_state=gps


# state별 제어
# - emergency : steer=yolotl, throttle은 현재값에서 emergency_deceleration_sec 동안
#               emergency_deceleration_target_throttle로 선형 감속
# - yolotl    : steer=yolotl, throttle=[yolotl_min, yolotl_max] 역비례 매핑
#               (yolotl로 전환 시 현재 throttle이 더 크면 decelerate_to_yolotl_sec 동안 선형 감속)
...
class DecisionNode(Node):
    def __init__(self):
        ...
        self.use_end_idx_topic = bool(self.get_parameter('use_end_idx_topic').value)
        ...
        self.get_logger().info(
            'decision3_node started: publish_rate=%.1fHz emergency_deceleration_sec=%.2fs '
            ...
            'end_idx=%d range=[%d,%d] use_end_idx_topic=%s topic=%s'
            % (
    ...
    def _maybe_log_status(self, steer_cmd: float, throttle_cmd: float):
        ...
        self.get_logger().info(
            ...
            f'lane_detect={self.lane_detect}, '
            f'num_lidar_cone={self.num_lidar_cone}, '
            f'end_idx={self.end_idx}, '
            f'end_idx_range=[{self.end_idx_min},{self.end_idx_max}], '
    ...
    def _on_timer(self):
        ...
        if state == MISSION_EMERGENCY:
            ...
            if prev_state != state:
                if self.current_auto_throttle > yolotl_target_throttle:
                    self._start_transition_deceleration(
                        now_sec=now_sec,
                        target_state=state,
                        target_throttle=yolotl_target_throttle,
                        duration_sec=self.decelerate_to_yolotl_sec,
                    )
                else:
                    self._cancel_transition_deceleration()
            if self.transition_decel_active and self.transition_decel_target_state == state:
                throttle_cmd = self._compute_transition_deceleration_throttle(now_sec)
            else:
                throttle_cmd = yolotl_target_throttle
